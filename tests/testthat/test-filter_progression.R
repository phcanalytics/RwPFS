...
test_that("filter_progression - Validations", {
  ...
  mock_fk_table <- tibble::tribble(
    ~patientid, ~startdate
  )


  testthat::expect_error(
    filter_progression(mock_fk_table,
    ...
    "diagnosisdateis not a column in your .progression_table (and must be)",
    fixed = TRUE
  )
  ...
  testthat::expect_error(
    ...
    "startdateis not of class date (and must be)",
    fixed = TRUE
  )

  mock_fk_table$startdate <- as.Date(mock_fk_table$startdate)
  testthat::expect_error(
    filter_progression(mock_fk_table,
      .start_date = "startdate",
    ...
    "Please specify whether radiographic confirmation of progression is required (TRUE/FALSE)",
    fixed = TRUE
  )
  testthat::expect_error(
    filter_progression(mock_fk_table,
    ...
    "Please specify whether to exclude progression events where pseudoprogression is mentioned (TRUE/FALSE)",
    fixed = TRUE
  )
  testthat::expect_error(
    filter_progression(mock_fk_table,
    ...
    "Please specify the number of days after baseline where progression events are ignored (0 for none)",
    fixed = TRUE
  )
  testthat::expect_error(
    filter_progression(mock_fk_table,
    ...
    "The label for output columns should be a string!",
    fixed = TRUE
  )
  testthat::expect_error(
    filter_progression(mock_fk_table,
      ...
      .exclude_pseudoprogression = TRUE,
      .discard_n_days = 100,
      .prog_filter_expression = FALSE
    ),
    "The excl_flag_expression argument should be either null or a string containing a logical expression using column names of the progression table",
    fixed = TRUE
  )
})
